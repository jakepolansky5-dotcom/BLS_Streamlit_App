...
def fetch_qcew_parallel(tasks, ownership_type, ind_codes):
    """Executes QCEW API calls in parallel threads."""
    results = []
    ...
    def task_worker(task):
        ...
        if not df_slice.empty:
            ...
            if not filtered.empty:
                filtered["Region"] = area_name
                filtered["Period"] = f"{yr}" if p_code == "a" else f"{yr} Q{p_code}"
                return filtered
        return None

    with concurrent.futures.ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(task_worker, t) for t in tasks]
    ...
    with col2:
        if run_sae:
            if not BLS_API_KEY:
                ...
                s_yr = sae_yr if sae_view == "Single Month Comparison Matrix" else sae_start
                e_yr = sae_yr if sae_view == "Single Month Comparison Matrix" else sae_end

                with st.spinner("Fetching SAE data..."):
                    st.session_state["df_sae"] = fetch_bls_batch(series_map, s_yr, e_yr, BLS_API_KEY)
                    st.session_state["sae_view"] = sae_view
                    st.session_state["sae_metric"] = selected_metric
                    st.session_state["sae_mo"] = sae_mo if sae_view == "Single Month Comparison Matrix" else None
                    st.session_state["sae_yr"] = sae_yr if sae_view == "Single Month Comparison Matrix" else None

        # Display Logic tied to session state
        if "df_sae" in st.session_state and not st.session_state["df_sae"].empty:
            df_res = st.session_state["df_sae"]
            s_view = st.session_state["sae_view"]
            s_metric = st.session_state["sae_metric"]

            if s_view == "Single Month Comparison Matrix":
                mo_map = {
                ...
                filtered = df_res[
                    (df_res["year"] == str(st.session_state["sae_yr"]))
                    & (df_res["period"] == mo_map[st.session_state["sae_mo"]])
                ]

                if not filtered.empty:
                    st.subheader(f"{s_metric} Matrix ({st.session_state['sae_mo']} {st.session_state['sae_yr']})")
                    pivot_df = filtered.pivot(index="Industry", columns="Area", values="Value")
                    fmt = "{:,.1f}" if "Employees" in s_metric else "${:,.2f}" if "$" in s_metric else "{:,.2f}"
                    st.dataframe(pivot_df.style.format(fmt, na_rep="N/A"), use_container_width=True)
                else:
                    st.warning("No data returned for selected month/year.")
            else:
                df_trends = df_res[df_res["period"].str.startswith("M") & (df_res["period"] != "M13")].copy()
                df_trends["Date_Dt"] = pd.to_datetime(df_trends["year"] + "-" + df_trends["period"].str.replace("M", ""), format="%Y-%m")
                df_trends["Month_Year"] = df_trends["Date_Dt"].dt.strftime("%b %Y")
                df_trends = df_trends.sort_values("Date_Dt")

                focus_ind = st.selectbox("Select Industry to Plot Across Regions:", selected_sae_industries)
                trend_df = df_trends[df_trends["Industry"] == focus_ind]
                chart_pivot = trend_df.pivot(index="Month_Year", columns="Area", values="Value")

                st.line_chart(chart_pivot)
                st.subheader("Raw Historical Table")
                fmt = "{:,.1f}" if "Employees" in s_metric else "{:,.2f}"
    ...
    with o_col2:
        if run_oews:
            if not BLS_API_KEY:
                ...
                s_yr = oews_year if oews_view == "Regional Comparison (Single Year)" else oews_start_yr
                e_yr = oews_year if oews_view == "Regional Comparison (Single Year)" else oews_end_yr

                with st.spinner("Fetching OEWS data..."):
                    st.session_state["df_oews"] = fetch_bls_batch(oews_series_map, s_yr, e_yr, BLS_API_KEY)
                    st.session_state["oews_view"] = oews_view
                    st.session_state["oews_metric"] = selected_oews_metric

        if "df_oews" in st.session_state and not st.session_state["df_oews"].empty:
            df_oews = st.session_state["df_oews"]
            o_view = st.session_state["oews_view"]
            o_metric = st.session_state["oews_metric"]

            if o_view == "Regional Comparison (Single Year)":
                st.subheader(f"{o_metric} Matrix")
                pivot_oews = df_oews.pivot(index="Occupation", columns="Area", values="Value")
                fmt = "${:,.2f}" if "$" in o_metric else "{:,.0f}"
                ...
                st.dataframe(
                    occ_trend[["year", "Area", "Occupation", "Value_Formatted"]].rename(
                        columns={"year": "Year", "Value_Formatted": "Value"}
                    ),
...
with tab3:
    ...
    with q_col1:
        ...
        if taxonomy_level == "Broad Supersectors & Aggregates":
            ...
            default_sel = [
                ...
                "336 - Transportation Equipment (Shipbuilding/Aerospace)",
            ]
        else:
        ...
        selected_qcew_industries = st.multiselect("Select Industries to Query:", options=list(active_dict.keys()), default=default_sel)
    ...
    with q_col2:
        if run_qcew:
            ...
            with st.spinner("Fetching QCEW slices in parallel..."):
                st.session_state["qcew_df"] = fetch_qcew_parallel(tasks, ownership_type, ind_codes)
                st.session_state["qcew_metric"] = selected_qcew_metric
                st.session_state["qcew_metric_col"] = metric_col
                st.session_state["qcew_active_dict"] = active_dict

        if "qcew_df" in st.session_state and not st.session_state["qcew_df"].empty:
            qcew_df = st.session_state["qcew_df"]
            m_col = st.session_state["qcew_metric_col"]
            m_label = st.session_state["qcew_metric"]
            dict_map = st.session_state["qcew_active_dict"]

            if m_col in qcew_df.columns:
                qcew_df[m_col] = pd.to_numeric(qcew_df[m_col], errors="coerce")
                inv_ind_map = {v: k for k, v in dict_map.items()}
                qcew_df["Industry_Label"] = qcew_df["industry_code"].map(inv_ind_map).fillna(qcew_df["industry_code"])

                st.subheader(f"Results: {m_label}")
                ...
                if not sub_df.empty:
                    piv_table = sub_df.pivot(index="Period", columns="Region", values=m_col)
                    st.line_chart(piv_table)

                    fmt_str = "${:,.2f}" if "$" in m_label or "pay" in m_col or "wage" in m_col else "{:,.0f}"
                    st.subheader(f"Data Table: {focus_q_ind}")
                    st.dataframe(piv_table.style.format(fmt_str, na_rep="N/A"), use_container_width=True)
                else:
                    st.warning("No data found for the selected industry.")
